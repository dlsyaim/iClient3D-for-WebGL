...
define([
    ...
    PinBuilder.prototype.fromUrl = function(url, color, size) {
        ...
        var modelUrl = buildModuleUrl('Assets/Textures/maki/' + encodeURIComponent(url) + '.png')
        return createMarker(modelUrl, undefined, color, size, this._cache);
    };
    ...
    function createMarker(url, label, color, size, cache) {
        //Use the parameters as a unique ID for caching.
        stringifyScratch[0] = url;
        stringifyScratch[1] = label;
        stringifyScratch[2] = color;
