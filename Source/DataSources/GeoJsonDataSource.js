...
define([
    ...
    function createPoint(dataSource, geoJson, crsFunction, coordinates, options) {
        ...
        if (defined(symbol)) {
            ...
            canvasOrPromise = dataSource._pinBuilder.fromUrl('blu-diamond', color, size);
        }
    ...
    GeoJsonDataSource.prototype.loadPoiJsonData = function(data, options) {
        //>>includeStart('debug', pragmas.debug);
        if (!defined(data)) {
            throw new DeveloperError('data is required.');
        }
        ...
        var that = this;
        return load(that, data, options, sourceUri);
    };
