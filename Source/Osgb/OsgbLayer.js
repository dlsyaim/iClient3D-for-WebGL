...
define([
     ...
     */
        function computerLength2Pix(radius,dDistance,gl){
            var dFov = 45;
     ...
     * @param gl
     */
        PagedLOD.prototype.calcPixFromCam = function(vCam,gl){
            if(!this._boundingSphere){
                return;
            }
            var vec = Cartesian3.clone(this._boundingSphere.center);
            var dis = Cartesian3.distance(vec,vCam);
            var npix = computerLength2Pix(this._boundingSphere.radius ,dis,gl);
            this._pix = npix;
            if(this._entity){
