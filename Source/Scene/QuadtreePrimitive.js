...
define([
    ...
    function selectTilesForRendering(primitive, context, frameState) {
        ...
        while (defined((tile = traversalQueue.dequeue()))) {
            ...
            /*if(!backfaceCull(tile,frameState)){
                continue;
            }*/
    ...
    function backfaceCull(tile,frameState){
        var surfaceTile = tile.data;
        var center = surfaceTile.center;
        var normal = surfaceTile.eastNormal;
        var campos = frameState.camera._positionWC;

        var x = campos.x - center.x;
        var y = campos.y - center.y;
        var z = campos.z - center.z;
        // normalize vDir:
        ...
        var bVisible = false;
        var normal = surfaceTile.eastNormal;
        var d = (x * normal.x + y * normal.y + z * normal.z);
        if (d>0.4)
        {
            bVisible = false;  // reject
            normal = surfaceTile.northNormal;
            d = (x * normal.x + y * normal.y + z * normal.z);
            if (d>0.4)
            {
                bVisible = false;  // reject
                normal = surfaceTile.southNormal;
                d = (x * normal.x + y * normal.y + z * normal.z);
                if (d>0.4)
                {
                    bVisible = false;  // reject
                    normal = surfaceTile.westNormal;
                    d = (x * normal.x + y * normal.y + z * normal.z);
                    if (d>0.4)
                    {
                        bVisible = false;  // reject
