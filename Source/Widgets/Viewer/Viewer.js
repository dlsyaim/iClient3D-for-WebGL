...
define([
     ...
     * @param {Boolean} [options.pathQuery=true] If set to false, the pathQuery widget will not be created.
...
Either specify options.terrainProvider instead or set options.baseLayerPicker to true.');
        ...
        //if (!scene3DOnly && (options.sceneModePicker !== false)) {
        if (!scene3DOnly && (options.sceneModePicker === true)) {
        ...
        var pathQuery;
        if (defined(options.pathQuery) && options.pathQuery !== false) {
            var pathQueryContainer = document.createElement('div');
            pathQueryContainer.className = 'pathQueryContainer';
            viewerContainer.appendChild(pathQueryContainer);
            pathQuery = new PathQuery(pathQueryContainer,this);
        }
